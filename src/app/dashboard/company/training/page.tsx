...
export default function CompanyTrainingPage() {
  ...
  const [previewModule, setPreviewModule] = useState<TrainingModule | null>(
    null
  );
  ...
  const totalSlides = modules.reduce(
    (sum, m) => sum + (m.slides?.length || 0),
    0
  );
  const totalQuestions = modules.reduce(
    (sum, m) => sum + (m.quiz?.length || 0),
    0
  );
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View and manage training modules for {companyName}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {modules.length === 0 ? (
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              handleDelete(module._id, module.meta.title)
                            }
